...
function colorSwap() {
    ...
    let colors = ['#F012BE', '#faa05a', '#f0506e', '#32d296', '#ffdd59', '#485460', '#3c40c6', '#05c46b', '#00d8d6'];
    let random_color = Math.floor(Math.random() * colors.length);
    let color_style = colors[random_color] + ' !important';
    let css = ":root { --main-color: " + color_style + "; } ";
...
// NAV MENU 
    $('#toggle').click(function () {
        ...
        // document.getElementById('shuffle_button').style.display = 'none';
    ...
    $('#overlay').click(function () {
        ...
        var _sb = document.getElementById('shuffle_button');
        setTimeout(function () {
            if (_sb.classList) {
                _sb.classList.toggle("invisible");
                setTimeout(function () {
                    _sb.classList.toggle("hidden");
                }, 80);
            } else {
                // For IE9
                var classes = _sb.className.split(" ");
                var i = classes.indexOf("hidden");
