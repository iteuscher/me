...
var modalbg = document.getElementById("modal-bg-1");

//     var initScroll = $(modalbg).scrollTop();
...
jQuery(document).ready(function ($) {
    ...
    $('.modal .cd-modal-close').on('click', function () {
        closeModal();
    });
    ...
    $(window).on('resize', function () {
        ...
        if ($('.modal.modal-is-visible').length > 0) window.requestAnimationFrame(updateLayer);
    });
    ...
    function animateLayer(layer, scaleVal, bool) {
        layer.velocity({
            ...
            (bool) ?
            layer.parents('.modal').addClass('modal-is-visible').end().off('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend'): layer.removeClass('is-visible').removeAttr('style').siblings('[data-type="modal-trigger"]').removeClass('to-circle');
        });

    }

    function updateLayer() {
        var layer = $('.modal.modal-is-visible').find('.cd-modal-bg'),
